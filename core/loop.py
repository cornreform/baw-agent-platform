...
def build_system_prompt(config: dict, data_dir: Optional[Path] = None,
    ...
    system_prompt += (
        ...
        f"\n## STRUCTURED CONTENT — Use Telegram Native Formatting\n"
        f"Your output uses Telegram's HTML parser. Use these formatting tools:\n"
        f"- <b>bold</b> — headers, labels\n"
        f"- <code>inline code</code> — file paths, commands, IDs\n"
        f"- <pre>multi-line\ncode block</pre> — longer code or structured data\n"
        f"- <i>italic</i> — emphasis\n"
        f"- <u>underline</u> — key points\n"
        f"- <s>strikethrough</s> — deprecated/cancelled items\n"
        f"- Markdown tables for tabular/structured data:\n"
        f"  | Header A | Header B |\n"
        f"  |----------|----------|\n"
        f"  | value 1  | value 2  |\n"
        f"  Tables render natively in Telegram — no ASCII box-drawing needed.\n"
        f"\n"
        f"## HARD GATE — NO ASCII BOX-DRAWING\n"
        f"NEVER use Unicode box-drawing characters for charts, tables, or diagrams:\n"
        f"  ─ ┌ ┐ └ ┘ │ ┤ ├ ┬ ┴ ┼ ═ ║ ╗ ╔ ╚ ╝ █ ▓ ▒ ░ ▄ ▀▐ ▌\n"
        f"Instead:\n"
        f"- Org charts → nested <b> + indented bullet points\n"
        f"- Tables → Markdown pipe tables\n"
        f"- Relationships → <code>parent > child</code> format\n"
        f"- Timelines → numbered list with <b>dates</b>\n"
