...
def build_system_prompt(config: dict, data_dir: Optional[Path] = None,
    ...
    if not quick_mode:
        ...
        system_prompt += (
            ...
            f"- mmx (MiniMax CLI): text, vision describe, speech, image generate, video, music, search\n"
            f"  Vision usage: mmx vision describe <image_path> --question \"...\"\n"
            f"- python3, pip, git, curl, wget, jq, ffmpeg, identify (ImageMagick)\n"
            f"  Use 'bash' tool to run these. You can register new CLI tools anytime.\n"
            f"\n## Tool extension (self-configuration)\n"
            f"- If the user tells you to use a new CLI tool, find its path with 'which <tool>' or 'find'\n"
            f"- Write a tool wrapper at ~/baw/tools/<name>.py (copy pattern from tools/vision.py)\n"
            f"- The tool auto-registers on next use. No restart needed.\n"
            f"\n## Dynamic context\n"
            f"- Current tone: {tone}\n"
...
def run_agent(
    ...
    _GOAL_PURSUIT_MAX_ATTEMPTS = 3  # Max full from-scratch re-plans (tight: prevent loop flood)
    _MAX_RECALCULATES = 3           # Max micro re-routes per pursuit (tight: 3 chances per plan)
    steps_completed = 0
