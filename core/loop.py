...
def build_system_prompt(config: dict, data_dir: Optional[Path] = None,
    ...
    if not quick_mode:
        ...
        system_prompt += (
            ...
            f"- 🔴 EXCEPTION: If the user explicitly says 'report progress' / '匯報進度' / '跟進' / '每步話俾我知', show ONE LINE per step (e.g. '🔧 installing npm package... → ✅ done'). Then final summary at the end.\n"
            f"- 🔴 After each step completes, immediately proceed to the next step. Do NOT wait, do NOT summarize, do NOT ask for permission.\n"
            f"- 🔴 Only speak when ALL steps are done. Present the final result only. No partial reports (unless user asked for progress).\n"
            f"- NEVER end your response with a question. Execute directly.\n"
            f"- 🚫 DO NOT just echo commands back to the user. If user asks to install/run something, ACTUALLY CALL THE TOOL. Saying '已執行安裝命令' without running bash is fabrication. Saying '你可以用呢個 command' without executing it is useless. Always: (1) CALL THE TOOL (2) report the real output.\n"
            f"- 🔴 After each step completes, immediately proceed to the next step. Do NOT wait, do NOT summarize, do NOT ask for permission.\n"
            f"- 🔴 Only speak when ALL steps are done. Present the final result only. No partial reports.\n"
            f"- NEVER end your response with a question. Execute directly.\n"
            f"- NEVER end your response with a question. Execute directly.\n"
            f"\n## 🔴 CHECKBOX RULE — User must be able to read result\n"
            f"When you report a step result, the output MUST make it OBVIOUS whether it succeeded or failed.\n"
            f"- ✅ means TOOL EXECUTED AND SUCCEEDED (tool returned exit code 0, real output shown)\n"
            f"- ❌ means TOOL EXECUTED AND FAILED (tool returned non-zero exit code, stderr shown)\n"
            f"- Do NOT use ✅ for planning steps — planning steps have no execution result yet\n"
            f"- When a step is NOT yet executed: just describe the step (NO emoji)\n"
            f"- When a step is running: use ▶ prefix\n"
            f"- After SUCCESSFUL install: ALWAYS show `✅ INSTALLED: <binary> (<version>) at <path>`\n"
            f"  The `install` tool already does this — just relay its exact output.\n"
            f"- After FAILED install: OUTPUT the exact error from the install tool.\n"
            f"  Include exit code + stderr. Never replace with just failed.\n"
            f"- If in doubt: include the raw tool output. MORE INFO > LESS INFO.\n"
            f"- NEVER end your response with a question. Execute directly.\n"
            f"\n## 🔴 CHECKBOX RULE — User must be able to read result\n"
            f"When you report a step result, the output MUST make it OBVIOUS whether it succeeded or failed.\n"
            f"- ✅ means TOOL EXECUTED AND SUCCEEDED (tool returned exit code 0, real output shown)\n"
...
def run_agent(
    ...
    plan_prompt = (
        ...
        "SPECIAL RULE: If the goal is to RUN a command (e.g. 'baw self-test', 'baw petrestaurants',\n"
                    ...
                    try:
                        ...
                        _inline_ctx = Context(
                            system_prompt=(
                                ...
                                "CRITICAL: If the step says 'Execute command' or 'Run command' or mentions\\n"
                                "a CLI command, generate subprocess.run() with the exact command:\\n"
                                ...
                                "Available imports:\n"
                                "- import subprocess, os, json, yaml, re, sys, datetime, time\n"
                                "- from pathlib import Path\n"
                                ...
                                "When a step says 'search memory', '搜尋記憶':\n"
                                "  USE memory_search(query=..., limit=5) directly.\n"
                                "When a step says 'read file', '讀檔', '讀取':\n"
                                "  USE read_file(path=...) directly.\n"
                                "When a step says 'write file', '寫檔', '寫入':\n"
                                "  USE write_file(path=..., content=...) directly.\n"
                                "NEVER write a plan/summary. CALL THE FUNCTION DIRECTLY and set _result.\n"
