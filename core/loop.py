...
def build_system_prompt(config: dict, data_dir: Optional[Path] = None,
    ...
    if not quick_mode:
        ...
        system_prompt += (
            ...
            f"You MUST NEVER change the user's model assignments, fallbacks, capabilities, or\n"
            f"tool methods (tts.method, vision.model, image_generation.model, etc.) without\n"
            f"explicit user permission. Your role is to:\n"
            f"  1. DETECT issues (missing keys, unreachable endpoints, etc.)\n"
            f"  2. REPORT them clearly to the user\n"
            f"  3. WAIT for the user to decide what changes to make\n"
            f"Do NOT use config(action=set) to change models, fallbacks, or methods unless\n"
            f"the user explicitly told you 'fix it', 'set it to X', or 'change Y to Z'.\n"
