#!/bin/bash
# BAW repo cleanup script — remove Hermes/the user/Sticky references
# Run from repo root: cd /home/user/baw && bash .cleanup.sh
...
sed -i 's/(the user|Sticky|Robi|BAW|Hermes)/(developer|assistant|BAW)/' tools/knowledge_graph.py
...
# SOUL.default.md template - replace the user references with generic
sed -i 's/the user 可以直接改呢個 file/BAW 管理員可以直接改呢個 file/' SOUL.default.md
sed -i 's/我叫 \*\*BAW\*\*（Black And White）。我係 the user 嘅 Agent Platform。/我叫 **BAW**（Black And White）。我係一個通用 Agent Platform。/' SOUL.default.md
sed -i 's/我係 the user 嘅助理/我係一個通用 AI 助理/' SOUL.default.md
sed -i 's/我嘅 naming 來自 the user 兩隻黑白色嘅狗/我嘅 naming 來自「黑白分明」嘅設計哲學/' SOUL.default.md
sed -i 's/同 the user 一齊生活/同用戶一齊生活/' SOUL.default.md
echo "  Cleaned: SOUL.default.md"

# config.sample.yaml
sed -i 's/日常同 the user 吹水/日常對話/' config.sample.yaml
sed -i 's/# the user only/##/g' config.sample.yaml
